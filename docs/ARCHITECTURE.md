...
```
 ...
 supported session selector or .jsonl path ────────┴──▶ [evidence projection]
                     ...
                     session/repo explicit apply invocation
                                    ▼
                            [orangu-apply skill] ──▶ application receipt ──▶ applied state
                                                                                     │
...
- **`src/report/client/`** — the browser renderer (TypeScript → inlined IIFE by `scripts/build.mjs`). Hand-rolled SVG charts, URL-hash state, cross-filtering. No runtime dependencies. Theme is `data-theme` only: light is the default everywhere and dark is opt-in through the toggle, a `?theme=dark` hash, or the landing's stored preference. Built as **three bundles** from three esbuild entries. One rule decides which bundle a surface belongs to: *anything written to a file must pass `scripts/assert-offline.mjs`, so the serve bundle can never back a file.*
  - **`CLIENT_JS`** (`client/main.ts`) — embedded in every session report. It carries a **two-sided ratchet** in `src/report/offline.test.ts`: a cap of **73,728 B** and, separately, an **exact length pin**. Any byte change to this bundle fails the suite until the pin is re-measured in the same commit; both numbers may only go down.
  - **`CLIENT_JS_SERVE`** (`client/serve-entry.ts`) — the same app plus the fetch/SSE data source and the `serveUi` injection seam. It carries network-API text by design, so it is served over loopback and never written to a file.
  - **`CLIENT_JS_AGG`** (`client/agg-entry.ts`) — the aggregate file report. Same app, filling `ServeUi` from data embedded in the file rather than from the network, so it renders the repo and global screens and the whole-harness block that a session report cannot. Because it is written to a file it passes the same offline gate as `CLIENT_JS`, and it carries its own size ratchet, born at its measured value and shrinking from there.

  Serve-only UI goes in the serve bundle, never the file ones. A string only the CLI needs (the mascot art) lives **outside** `src/report/client/`: esbuild does not treat a `String.raw` tagged template as side-effect-free, so such a literal survives tree-shaking and would otherwise ship inside every report.
- **`src/cli/`** — `report / analyze / list / repo / global / watch / serve / evidence / estimate / suggest / harness`; `--json` is redacted by default and `--slim` is available for legacy bounded consumers. `evidence` is the canonical skill seam. New verbs register in `src/cli/commands/` without touching `main.ts`.
- **The aggregate HTML report** — `orangu repo` and `orangu global` render their scope through `renderAggregateReport` in `src/report/render.ts`, beside `renderReport`: the same document skeleton, the same CSP, no `<link>`, and `CLIENT_JS_AGG` inlined. `--html <file>` writes it where you say; `--open` writes it into the temp directory as `orangu-<scope>-<hash>.html` and hands it to the browser, which is what the dashboard's repository and global choices ask for. Both are refused with `--json`, a machine read that has no side effect. **`--out <file>` still writes the aggregate JSON, always** — it is the contract `/orangu:harness` reads back, so it can never become HTML. The written file goes through `src/cli/private-output.ts` (mode `0600`, symlink-refusing), is redacted by default, and passes the same zero-network gate as the session report.
...
`scripts/build.mjs` uses esbuild to (1) bundle the report client into a TS string module — all three entries (`main.ts`, `serve-entry.ts`, `agg-entry.ts`) into `CLIENT_JS`, `CLIENT_JS_SERVE` and `CLIENT_JS_AGG`, with client CSS = `src/report/client/tokens.css` + `styles.css`, (2) bundle the CLI to `dist/orangu.js`, (3) bundle the Claude plugin's offline CLI to `plugin/bin/`, and (4) copy that same CLI plus canonical brand assets into `plugins/orangu/`. Zero runtime dependencies ship; esbuild/vitest/typescript are dev-only.
