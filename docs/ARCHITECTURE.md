...
- **`plugin/skills/orangu-improve/` and `plugin/skills/orangu-apply/`** — the Claude Code draft and explicit application workflows. `plugin/skills/orangu-suggest/` is a compatibility alias that forwards to improve. Supporting analyze, mega, and watch skills remain separate.
- **`.agents/skills/orangu-improve/` and `.agents/skills/orangu-apply/`** — Codex repo-skill variants of the same evidence, artifact, scope, application, and session-verification contracts. They do not add a Codex transcript adapter.
...
`scripts/build.mjs` uses esbuild to (1) bundle the report client into a TS string module (client CSS = `src/report/client/tokens.css` + `styles.css`), (2) bundle the CLI to `dist/orangu.js`, (3) bundle the plugin's offline CLI to `plugin/bin/`. Zero runtime dependencies ship; esbuild/vitest/typescript are dev-only.
