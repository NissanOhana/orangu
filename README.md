...
One real number, from one local session: 34.9M of the 37.0M tokens the model processed were cache reads, one 223k-token context read again 156 times over. Tokens are the only usage metric orangu has, because they are the only one the transcript records.
