...
test('localhost creates only a copy handoff and never offers automatic model launch', async ({ page, context }) => {
  ...
  await expect(row.locator('.kick-msg')).toContainText('Claude copied')
  ...
  await expect(handoffs.nth(0)).toHaveAttribute('data-copy', /claude "\/orangu:improve/)
  await expect(handoffs.nth(1)).toHaveAttribute('data-copy', /^\$orangu-improve /)
  await handoffs.nth(1).click()
  await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toContain('$orangu-improve')
  expect(errors).toEqual([])
})
...
test('localhost saved proposals are escaped, status-distinct, responsive, and expose copy-only apply handoffs', async ({ page, context }) => {
  ...
  await expect(copyButtons.nth(0)).toHaveAttribute('data-copy', `claude "/orangu:apply ${proposedId}"`)
  await expect(copyButtons.nth(1)).toHaveAttribute('data-copy', `$orangu-apply ${proposedId}`)
  await copyButtons.nth(1).click()
  await expect.poll(() => page.evaluate(() => navigator.clipboard.readText())).toBe(`$orangu-apply ${proposedId}`)
