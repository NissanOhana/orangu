...
- Session-scope applications may later be verified against a discoverable supported session from the same canonical workspace. Repo-scope applications remain `applied` until Orangu can compare later repository sessions. Global-scope proposals cannot be applied.
