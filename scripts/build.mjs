...
// aggregate-report bundle: same source, third entry. This one is written into a FILE, so it must
// stay as offline-clean as the file-mode bundle above (offline.test.ts greps it too); it exists
// only because it must never import serve-ui.ts, which carries fetch/EventSource by design.
const aggEntry = join(root, 'src/report/client/agg-entry.ts')
let aggJs = ''
if (existsSync(aggEntry)) {
  const r = await build({
    entryPoints: [aggEntry],
    bundle: true,
    minify: true,
    format: 'iife',
    target: ['es2020'],
